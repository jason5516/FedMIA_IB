                ...
                for batch_idx, (x, y) in enumerate(train_ldr):
                    ...
                    if self.defense=='mix_up':
                        ...
                        if self.ib:
                            ...
                            loss_all = kl_loss * self.model.beta + ce_loss
                            loss += loss_all
    ...
    def test(self, dataloader):
        ...
        with torch.no_grad():
            for load in dataloader:
                ...
                if self.ib:
                    pred, kl_loss = self.model(data)  # test = 4
                    ce_loss = F.cross_entropy(pred, target, reduction='sum').item()
                    loss_all = self.model.beta * kl_loss + ce_loss
                    loss_meter += loss_all
                    all_kl_loss += kl_loss.cpu().item()
                    all_ce_loss += ce_loss
                else:
                    pred = self.model(data)  # test = 4
                    ce_loss = F.cross_entropy(pred, target, reduction='sum').item()
                    loss_meter += ce_loss #sum up batch loss
                    all_ce_loss += ce_loss
                pred = pred.max(1, keepdim=True)[1] # get the index of the max log-probability
                acc_meter += pred.eq(target.view_as(pred)).sum().item()
                runcount += data.size(0) 
        
        all_kl_loss /= runcount
        all_ce_loss /= runcount
        loss_meter /= runcount
        acc_meter /= runcount

        return  loss_meter, all_kl_loss, all_ce_loss, acc_meter
