...
def lira_attack_ldh_cosine(f,epch,K,iid,attack_mode="cos"):
    # attack_mode="cos"
    # training_res = save_dict
    mix_length=int(10000/K)
    if iid:
        select_mode = 0
        select_method = "none"
    else:
    ...
    # get target result and shadow result
    target_res = training_res[target_idx]
    shadow_res = training_res[:target_idx] + training_res[target_idx + 1:]
    ...
    if epch % 10 ==0:
        ...
        print(f"tprs:{tprs}, auc: {auc}, log10_auc: {log_auc}")

    return accs,tprs,auc,log_auc,(train_l_out,test_l_out)

def cos_attack(f,K,epch,attack_mode):
...
def fig_out(x_axis_data, MAX_K,defence,seed,log_path, d,avg_d=None,single_score=None, other_scores=None,accs=None): 
    ...
    log_path="/".join(log_path)+f"/attack_score_{select_mode}_{select_method}_n{SHADOW_NUM}_s{SEED}_attack_id{ATK_ID}.log"
...
def main(argv):
    global MODE, attack_modes, PATH, p_folder, device, select_mode, select_method, SHADOW_NUM, SEED, mix_length, ATK_ID
    global SAVE_DIR
