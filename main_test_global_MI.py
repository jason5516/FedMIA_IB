...
class FederatedLearning(Experiment):
    """
    Perform federated learning with Mutual Information (MI) calculation
    
    MI計算方法說明：
    1. 預設方法：使用 self.model.features 來獲取層特徵值（推薦）
       - 適用於 ResNetwithIB_layer 等具有 features 屬性的模型
       - 更高效，記憶體使用較少
    
    2. 可選方法：使用 forward hooks 來捕獲層特徵值
       - 在 _local_update_with_mi_calculation 中設置 use_hooks=True
       - 適用於需要更細粒度控制的情況
       - 可能消耗更多記憶體
    
    使用範例：
    - 預設使用 model.features：use_hooks=False（預設值）
    - 使用 hooks 方法：use_hooks=True
    """
    ...
    def register_feature_hooks(self, model):
        """註冊forward hooks來捕獲每層的輸出特徵值，用於每個batch的MI計算（可選方法）"""
        self.feature_hooks = {}
        ...
        def create_hook(layer_name):
            def hook_fn(module, input, output):
                if self.record_features:
                    ...
                    # 確保按照層的順序添加
                    layer_order = ['layer1', 'layer1.0.ib', 'layer1.0.bn2', 'layer1.1', 'layer1.1.ib', 'layer1.1.bn2', 'layer2', 'layer2.1.conv2', 'layer3', 'layer3.1.conv2', 'layer4', 'layer4.1.conv2', 'linear']
        ...
        for name, module in model.named_modules():
            # 試試bn前特徵
            if name in ['layer1', 'layer1.0.ib', 'layer1.0.bn2', 'layer1.1', 'layer1.1.ib', 'layer1.1.bn2', 'layer2', 'layer2.1.conv2', 'layer3', 'layer3.1.conv2', 'layer4', 'layer4.1.conv2', 'linear']:
                layer_order.append(name)
                hook = module.register_forward_hook(create_hook(name))
                self.feature_hooks[name] = hook
    
    def remove_feature_hooks(self):
        """移除所有的forward hooks（可選方法）"""
        for hook in self.feature_hooks.values():
            hook.remove()
        self.feature_hooks = {}
    
    def enable_hook_based_mi_calculation(self):
        """啟用基於hook的MI計算方法（可選）
        
        使用此方法可以選擇使用hook方式來捕獲層特徵值，而不是使用model.features
        適用於需要更細粒度控制特徵值捕獲的情況
        """
        print("警告：啟用hook方式的MI計算。這是可選功能，預設使用model.features更高效。")
        return True
    ...
    def _local_update_with_mi_calculation(self, dataloader, local_ep, lr, optim_choice, sampling_proportion, epoch, client_id, use_hooks=False):
        """帶有MI計算的本地訓練函數，每個batch計算一次MI
        
        Args:
            use_hooks: 是否使用hook方法記錄特徵值（可選，預設為False，使用model.features）
        """
        self.model.train()
        ...
        if use_hooks:
            ...
            print(f"使用hook方法記錄客戶端 {client_id} 在第 {epoch + 1} 輪的層特徵值")
        ...
        for ep in range(local_ep):
            ...
            for batch_idx, (x, y) in enumerate(train_ldr):
                ...
                try:
                    ...
                    # 即時顯示MI結果
                    MI_results[f'iter{batch_count}'] = {'IXZ':I_XT, 'IZY':I_TY}
                    # tqdm.write(
                    #     f"客戶端 {client_id}, Epoch {epoch+1}, Batch {batch_idx+1}: \n"
                    #     f"  I(X;T): {I_XT.detach().cpu().numpy()} \n"
                    #     f"  I(T;Y): {I_TY.detach().cpu().numpy()} \n"
                    #     "-" * 50
                    # )
            ...
            print("-" * 50)
            self.save_mi_results(epoch+1, client_id, MI_results)
        ...
        if use_hooks:
            ...
            print(f"完成客戶端 {client_id} 在第 {epoch + 1} 輪的特徵值記錄")
    ...
    def train(self):
        ...
        for epoch in range(self.epochs):
            ...
            for idx in tqdm(idxs_users, desc='Epoch:%d, lr:%f' % (epoch, self.lr)):
                ...
                # 設置特徵值記錄標誌
                self.record_features = should_record_features
                self.current_client_id = idx if should_record_features else None
                ...
                if self.args.defense == 'FedDPA':
                    ...
                    if should_record_features:
                        # 預設使用model.features方法，可選擇使用hooks（use_hooks=True）
                        local_w, local_loss = self._local_update_with_mi_calculation(
                            local_train_ldrs[idx], self.local_ep, self.lr, self.optim,
                            args.sampling_proportion, epoch, idx, use_hooks=True
                        )
                    else:
                        local_w, local_loss = self.trainer._local_update_noback(local_train_ldrs[idx], self.local_ep, self.lr, self.optim, args.sampling_proportion)
